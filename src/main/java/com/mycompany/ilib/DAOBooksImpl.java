...
public class DAOBooksImpl extends Database implements DAOBooks {
    ...
    public List<Books> listar(String title,int page, int limit) throws Exception {
        ...
        try {
            String query;
            this.Conectar();
            if (title.isEmpty() && page != 0 && limit != 0) {
                query = "SELECT * FROM books limit " + (page - 1) * limit + " ," + limit;
            } else if(title.isEmpty() && page == 0 && limit == 0){
    ...
    public List<Books> listarId(String id) throws Exception {
        ...
        try {
            this.Conectar();
            String Query = id.isEmpty() ? "SELECT * FROM books;" : "SELECT * FROM books WHERE id LIKE '%" + id + "%';";
