...
public class DAOLendingsImpl extends Database implements DAOLendings {
 @Override
    public int contarRegistros() throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("select count(*) from lendings");
    ...
    @Override
    public List<Lendings> listar(int page, int limit) throws Exception {
        List<Lendings> lista = null;
        try {
            this.Conectar();
            String query = "";
            if (page == 0 && limit == 0) {
                query = "SELECT * FROM lendings ORDER BY id DESC";
            } else {
                query = "SELECT * FROM lendings ORDER BY id DESC limit " + (page - 1) * limit + " ," + limit;
            }
